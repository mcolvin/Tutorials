---
title: "Robust design-survival only."
...
and 7 secondary occasions.  Additionally, we are going to fix survival (phi; $\phi$) and capture probability ($p$)
to 0.8 and 0.4 respectively.  Let's assign those for the simulation.
...
p<-0.4
```
One thing the keep in mind is that $\phi$ and $p$ are assumed to be constant.  Therefore $\phi$ does not
vary among primary sampling occasions and $p$ does not vary among secondary occasions. This is a simplifying
...
In the super population there are 200 'spots' an individual can occupy.
Because there are 100 individuals the 'occupancy' rate is 0.5.
...
Hence the multiplication of $\phi$ by ZZ to account for whether a fish was previously alive and Z that the fish is
included in the super population.
